...
### 3.18 Collections, payment run, credit control — built (the Cash page)

**Collections (item 13).** `cashflow` action `collections`: ageing buckets over the open invoices, and "who to chase this week" — an invoice is chased when overdue by the calendar OR when the customer is past their OWN learned habit (layer-6 payment lags: median/p90 days from invoice date, read from `bk_check_proposals` as evidence at any status). Ranked by money × lateness, each with a plain-English reason. When actually overdue, one click sends **Zoho's own payment reminder** — `POST /invoices/{id}/paymentreminder` needs `to_mail_ids` spelled out (verified live; an empty body fails with a misleading "email not found"); recipients come from the invoice's contact persons, else the contact email; no email → a polite refusal saying what to add. Nothing is scheduled or bulk; every send is audited.

**Payment run (item 14).** Action `payment_run`: everything overdue plus bills due within `payment_run_horizon_days` (7), a bill without terms due on its own date, grouped per vendor with the most urgent first; a vendor holding unused credit is flagged ("apply it before paying cash"). The reviewer unticks, picks the bank account and date; `record_payments` re-validates every amount against the CURRENT open bills (never the client's numbers — over-limit, wrong vendor, no-longer-open, and locked-period dates all refuse with the reason), then posts one `POST /vendorpayments` per vendor with `bills[]`. Recorded payments are audited AND count as "recorded through our app" for the bank-statement engine (the loader reads the `vendor_payment_recorded` audit rows), so the eventual statement line links instead of double-recording. Recording a payment ≠ moving money at the bank; the UI says so.

**Credit control (item 15).** Before a sales invoice goes out, `zoho-approve` checks the customer's exposure (fresh `outstanding_receivable_amount` + this invoice) against their limit — Zoho's own `credit_limit` when the org enables the feature (the toggle is UI-only on the .ae DC, code 75100), else the app-side map `company_config.credit_limits`. Over the limit → 409 with the numbers and the unused-credit hint; a human override (reason, audited) still posts; the credit picture rides on every approve response. The Cash page's credit watch lists customers over or near (headroom < 25% of exposure) their limit. `zoho-pull` caches contact `credit_limit`/outstanding/unused-credits/email alongside the TRN. Pure logic in `cashflow/cash.ts`; test `cashflow-accuracy`; all three verified live (reminder sent to a reserved-domain address; 2,000 vendor payment posted and the bill paid; standalone 409 → override → posted).

### 3.19 Fixed assets + prepayment/accrual schedules — built

**Follow-ups from a bill.** After `zoho-approve` pushes a bill, its lines are read against the chart of accounts: a line on a `fixed_asset` account proposes an **asset record** (`bk_asset_proposals`, unique per bill × line — re-approving never duplicates); a line on a current-asset account named "Prepaid …" proposes a **prepayment schedule** (`bk_schedules`, total from the line, 12 months from the bill month, the P&L account deliberately left for the reviewer). The approve response carries `followups`.

**Fixed assets.** `create_asset` posts the proposed record to Zoho's Fixed Assets module — only on a click, Zoho's message verbatim on refusal. Verified live with the module on: `POST /fixedassets` `{asset_name, fixed_asset_type_id, asset_account_id, asset_cost, dep_start_value, asset_purchase_date, depreciation_start_date}` (`total_life` is MONTHS; the asset type template — `POST /fixedassettypes` with `expense_account_id` + `depreciation_account_id` — supplies the depreciation defaults); Zoho creates it as a DRAFT, and since the reviewer's click is the confirmation, the connector activates it (`POST /fixedassets/{id}/status/active`). NOTE: like retainers, the module's GETs soft-succeed while creates return 10002 until the org enables Fixed Assets in Zoho preferences. Month-end also asks **"did depreciation run?"** — when the register has active assets and no journal this month mentions depreciation → attention nudge pointing at Zoho's Record Depreciation.

**Schedules.** A schedule spreads `total` over `months` from `start_period` — equal amounts, rounding tail on the last month. Each due month becomes a journal proposal (kind `schedule`, unique per schedule × period): Dr the P&L account / Cr the balance-sheet account — the same shape releases a prepaid asset and builds an accrued liability. Activation validates (accounts differ and exist, 1–60 months, total > 0); accruals are added by hand on the Month-end card; posting rides the existing confirm-to-post machinery (validation, period lock, never twice), and a fully-posted schedule flips to `done`. Index note: the one-per-kind-per-period unique index is scoped `pattern_id is null AND schedule_id is null` — several schedules may share a period. `month-end/schedules.ts`; test `assets-schedules-accuracy`; verified live (laptop + insurance bill → both follow-ups proposed; June release posted to Zoho: Dr expense 100 / Cr T1 Prepaid Insurance @ 2026-06-30; progress 1/12; manual accrual proposing 500/month).

### 3.20 Books hygiene + statement evidence — built

**Hygiene (month-end card, `month-end/hygiene.ts`).** Lists with plain-English notes; nothing changes on its own: suspense/uncategorised balances (name-matched accounts + Opening Balance Adjustments with a non-zero balance — the only hygiene item that raises an ATTENTION nudge); doubled-up vendors/customers (same normalised name — dots removed before splitting so "L.L.C." reads as one word, legal suffixes stripped — or one TRN on two contacts; merge advice); missing TRNs (VAT-registered contacts without a valid 15-digit TRN — Form 201 and the e-invoice both need it); duplicate ledger accounts (normalised name); unused accounts (user-created, active, zero balance, AND confirmed never-posted via `GET /chartofaccounts/transactions?account_id=` — a zero balance alone can just mean fully settled; capped at 30 checks per run). Test `hygiene-evidence-accuracy`; verified live (parked 50 in T1 Suspense caught; the deliberate "L.L.C" duplicate caught; 4 never-posted accounts confirmed).

**Statement evidence on pushed records (`bank-statement/evidence.ts`).** Every file-mode line pushed to Zoho gets the statement attached to the record it created — customer/vendor payments and retainer receipts via `…/attachment`, expenses via `…/receipt`, deposits/transfers via `banktransactions/…/attachment`; refund records keep their own document links. The uploaded statement file travels when one exists; pasted/emailed statements (no file) get a one-page PDF note carrying the line and its context — Zoho refuses `.txt` ("File types with extension txt are not supported", verified live), so the note is generated as a real PDF (`textToPdf`, no dependencies). ANY failure on the file path (missing object, refused extension) falls back to the PDF note — the evidence must not vanish because the file could not travel. Best effort after the record exists; the push result reports `attached` per line and the Bank page says how many. Verified live both ways (PDF note and the statement PDF itself, visible on the Zoho expenses).
